...
export const getGroupAvailableTimeSlots = (
  date: Date,
  staffServicePairs: StaffServicePair[],
  staffSchedules: Record<string, StaffSchedule>,
  bookedAppointments: BookedAppointment[] = [],
  sameStaff: boolean = false
): string[] => {
  if (staffServicePairs.length === 0) return [];

  if (sameStaff) {
    // For same staff, find overlapping available slots
    const staffId = staffServicePairs[0].staffId;
    const totalDuration = staffServicePairs.reduce((sum, pair) => sum + pair.duration, 0);
    const staffSchedule = staffSchedules[staffId];
    ...
    // For different staff, find slots where ALL staff are available
    let commonSlots: string[] = [];
    
    for (const pair of staffServicePairs) {
      const staffSchedule = staffSchedules[pair.staffId];
      if (!staffSchedule) return []; // If any staff schedule is missing, no slots available
      
      const staffSlots = getAvailableTimeSlotsForDate(
        date,
        staffSchedule,
        pair.duration,
        bookedAppointments,
      ...
      if (commonSlots.length === 0) {
        ...
        commonSlots = commonSlots.filter(slot => {
          // Check if this slot works for all other staff members
          return staffServicePairs.every(otherPair => {
            if (otherPair.staffId === pair.staffId) return true;
            
            const otherStaffSchedule = staffSchedules[otherPair.staffId];
            if (!otherStaffSchedule) return false;
            
            const otherStaffSlots = getAvailableTimeSlotsForDate(
              date,
              otherStaffSchedule,
              otherPair.duration,
              bookedAppointments,
            ...
            return otherStaffSlots.includes(slot);
          });
        });
      }
    }
...
export const getGroupBookingDuration = (
  staffServicePairs: StaffServicePair[],
  sameStaff: boolean
): number => {
  if (sameStaff) {
    // Sequential booking - sum all durations
    return staffServicePairs.reduce((sum, pair) => sum + pair.duration, 0);
  } else {
    // Concurrent booking - maximum duration
    return Math.max(...staffServicePairs.map(pair => pair.duration), 0);
  }
};